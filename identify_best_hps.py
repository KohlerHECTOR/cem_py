...
def find_best_hyperparameters():
    ...
    best_reinforce = max(reinforce_results.items(), key=lambda x: x[1]) if reinforce_results else None
    best_reinforce_baseline = max(reinforce_baseline_results.items(), key=lambda x: x[1]) if reinforce_baseline_results else None
    ...
    print(f"CEM: {best_cem[0]} with score {best_cem[1]:.4f}" if best_cem else "No CEM data found")
    print(f"REINFORCE: {best_reinforce[0]} with score {best_reinforce[1]:.4f}" if best_reinforce else "No REINFORCE data found")
    print(f"REINFORCE+baseline: {best_reinforce_baseline[0]} with score {best_reinforce_baseline[1]:.4f}" if best_reinforce_baseline else "No REINFORCE+baseline data found")
    print(f"ES: {best_es[0]} with score {best_es[1]:.4f}" if best_es else "No ES data found")
