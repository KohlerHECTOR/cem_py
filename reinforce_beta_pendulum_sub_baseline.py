...
def update_policy(batch_trajectories, params, learning_rate, action_bounds):
    ...
    # Subtract baseline from returns for variance reduction
    ...
    grad_alpha = (common_term - digamma(alpha) + np.log(scaled_actions)) * advantage
    grad_beta = (common_term - digamma(beta) + np.log(1 - scaled_actions)) * advantage
...
def reinforce(
    ...
    """Train policy using parallel batch updates; adding entrop regul changes the objective"""
    # Create a unique folder name based on parameters
    folder_name = f"beta_reinforce_sub_baseline_data_Pendulum-v1_batch{batch_size}_lr{learning_rate}_evals{n_evals}_seed{seed}"
