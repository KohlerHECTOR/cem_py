...
def compute_mean_performance(x_data_list, y_data_list):
    ...
    # Find common x range
    valid_runs = [
        (x, y) for x, y in zip(x_data_list, y_data_list) if len(x) > 0 and len(y) > 0
    ]
    if not valid_runs:
        return None, None, float("-inf")
...
for i, std_ in enumerate([True]):
    for j, n_pop in enumerate([8, 16, 32, 64]):
        for k, elite in enumerate([0.125, 0.25]):
            ...
            color = sns.color_palette("OrRd", n_colors=len(combs) * 2)[
                len(combs) + c_idx
            ]
...
for i, lr in enumerate([0.0001, 0.001, 0.01, 0.1]):
    for j, evals in enumerate([1, 2, 50]):
        for k, batch_size in enumerate([8, 16, 32, 64]):
            ...
            color = sns.color_palette("GnBu", n_colors=len(combs) * 2)[
                len(combs) + c_idx
            ]
            x_data, y_data = plot_data(data_paths, "-", color)
...
for i, lr in enumerate([0.0001, 0.001, 0.01, 0.1]):
    for j, evals in enumerate([1, 50]):  # From the shell script
        for k, batch_size in enumerate([8, 16, 32, 64]):  # From the shell script
            ...
            color = sns.color_palette("Greens", n_colors=len(combs) * 2)[
                len(combs) + c_idx
            ]
            x_data, y_data = plot_data(data_paths, "-", color)
...
for i, lr in enumerate([0.0001, 0.001, 0.01, 0.1]):  # Adjust parameters as needed
    for j, noise_std in enumerate([0.1, 1, 0.01]):  # Adjust parameters as needed
        for k, n_pop in enumerate([8, 16, 32, 64]):  # Adjust parameters as needed
            ...
            color = sns.color_palette("Purples", n_colors=len(combs) * 2)[
                len(combs) + c_idx
            ]
            ...
            for data_path in data_paths:
                try:
                    ...
                    train_samples = data[
                        "total_samples"
                    ]  # These are already cumulative for ES

                    num_points = 100
                    indices = np.linspace(
                        0, train_scores.shape[0] - 1, num_points, dtype=int
                    )

                    train_scores = train_scores[indices]
                    train_samples_es = train_samples[
                        indices
                    ]  # Use directly without cumsum
            ...
            # Compute mean performance for this hyperparameter configuration
            config_x, config_y, config_score = compute_mean_performance(
...
if cem_best_config_x is not None and cem_best_config_y is not None:
    plt.plot(
        ...
        color="#e85c47",
        linewidth=4,
        label="CEM (best config)",
    )

if reinforce_best_config_x is not None and reinforce_best_config_y is not None:
    plt.plot(
        reinforce_best_config_x,
        reinforce_best_config_y,
        linestyle="dashed",
        color="#4878cf",
        linewidth=4,
        label="REINFORCE (best config)",
    )

if (
    reinforce_baseline_best_config_x is not None
    and reinforce_baseline_best_config_y is not None
):
    plt.plot(
        reinforce_baseline_best_config_x,
        reinforce_baseline_best_config_y,
        linestyle="dashed",
        color="#2ca02c",
        linewidth=4,
        label="REINFORCE+baseline (best config)",
    )

if new_baseline_best_config_x is not None and new_baseline_best_config_y is not None:
    plt.plot(
        new_baseline_best_config_x,
        new_baseline_best_config_y,
        linestyle="dashed",
        color="#9467bd",
        linewidth=4,
        label="ES (best config)",
    )
...
legend_elements = [
    ...
    Line2D(
        [0], [0], color="#e85c47", linestyle="dashed", lw=3, label="CEM (best config)"
    ),
    ...
    Line2D(
        [0], [0], color="#9467bd", linestyle="dashed", lw=3, label="ES (best config)"
    ),
]
plt.legend(
