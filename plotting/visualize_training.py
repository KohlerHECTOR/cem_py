...
def episodes(weights_file, hidden_size=16, num_episodes=100):
    ...
    params = reconstruct_policy_params(
        best_weights, input_size, hidden_size, output_size
    )
...
def plot_policy_evolution(
    weights_file, hidden_size=16, resolution=100, output_gif="policy_evolution.gif"
):
    """
    ...
    for iter_idx in tqdm(range(num_iterations)):
        ...
        plt.title(
            f"Total Samples: {total_samples}, Score: {best_score:.2f}", fontsize=18
        )
...
def plot_policy_evolution_reinforce(
    weights_file,
    hidden_size=16,
    resolution=100,
    output_gif="policy_evolution_reinforce.gif",
):
    """
    Create a GIF showing how the policy evolves throughout training.
    ...
    for iter_idx in tqdm(range(0, num_iterations, num_iterations // 100)):
        ...
        # Reconstruct policy parameters
        params = reconstruct_policy_params(
            training_data["params"][iter_idx], input_size, hidden_size, output_size
        )
...
def plot_policy_evolution_comparison(
    cem_weights_file,
    reinforce_weights_file,
    hidden_size=16,
    resolution=100,
    output_gif="policy_comparison.gif",
):
    """
    ...
    for frame_idx in tqdm(range(num_frames)):
        ...
        # Plot CEM policy
        cem_params = reconstruct_policy_params(
            cem_best_weights, input_size, hidden_size, output_size
        )
        ...
        reinforce_params = reconstruct_policy_params(
            reinforce_weights, input_size, hidden_size, output_size
        )
        ...
        for i, th in enumerate(theta):
            for j, th_dot in enumerate(theta_dot):
                ...
                sin_th = np.sin(th)
                state = np.array([cos_th, sin_th, th_dot])
                action, _, _, _, _ = sample_action(
                    state, reinforce_params, action_bounds
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gif", action="store_true", help="Create a GIF of policy evolution"
    )
    parser.add_argument(
        "--output_gif",
        type=str,
        default="policy_evolution.gif",
    ...
    if args.compare and args.weights_reinforce is not None:
        ...
        if args.weights_reinforce is not None:
            plot_policy_evolution_reinforce(args.weights_reinforce)
        else:
            plot_policy_evolution(
                args.weights, hidden_size=args.hidden_size, output_gif=args.output_gif
            )
    else:
