...
all_data_reinforce = [
    np.load(p, allow_pickle=True).item() for p in reinforce_data_paths
]
...
all_data_reinforce_baseline = [
    np.load(p, allow_pickle=True).item() for p in reinforce_baseline_data_paths
]
all_seeds_weights_reinforce_baseline = [
    d["params"] for d in all_data_reinforce_baseline
]
...
cem_scores = np.array(
    [
        [
            all_data_cem[s]["train_scores"][all_data_cem[s]["best_pop_idx"][i]]
            for i in range(len(all_data_cem[s]["best_pop_idx"]))
        ]
        for s in range(len(all_data_cem))
    ]
).flatten()
reinforce_scores = np.array([d["train_scores"] for d in all_data_reinforce])
reinforce_scores = np.array(
    [
        r[idxs]
        for r, idxs in zip(
            reinforce_scores,
            [
                np.linspace(0, len(s) - 1, 400, dtype=np.uint32)
                for s in all_seeds_weights_reinforce
            ],
        )
    ]
...
all_scores_for_interp = np.concatenate(
    [cem_scores, reinforce_scores, reinforce_bs_scores, all_cem_pop_scores]
)
all_weights_for_interp = np.vstack(
    [cem_reshaped, reinforce_reshaped, reinforce_bs_reshaped, all_cem_pop_weights]
)
...
grid_z = griddata(
    all_weights_2d_interp,
    all_scores_for_interp,
    (grid_x, grid_y),
    method="linear",
...
cem_2d = trajectory_points_2d[:n_cem].reshape(
    weights_cem.shape[0], weights_cem.shape[1], 2
)
reinforce_2d = trajectory_points_2d[n_cem : n_cem + n_reinforce].reshape(
    weights_reinforce.shape[0], weights_reinforce.shape[1], 2
)
reinforce_bs_2d = trajectory_points_2d[n_cem + n_reinforce :].reshape(
    weights_reinforce_bs.shape[0], weights_reinforce_bs.shape[1], 2
)
...
for i in range(reinforce_2d.shape[0]):
    ...
    plt.plot(
        reinforce_2d[i, 0, 0], reinforce_2d[i, 0, 1], "bo", markersize=6
    )  # Start point
    plt.plot(
        reinforce_2d[i, -1, 0], reinforce_2d[i, -1, 1], "bx", markersize=10
    )  # End point

# Plot REINFORCE with baseline trajectories
for i in range(reinforce_bs_2d.shape[0]):
    plt.plot(
        reinforce_bs_2d[i, :, 0], reinforce_bs_2d[i, :, 1], "g-", alpha=1, linewidth=2
    )
    plt.plot(
        reinforce_bs_2d[i, 0, 0], reinforce_bs_2d[i, 0, 1], "go", markersize=6
    )  # Start point
    plt.plot(
        reinforce_bs_2d[i, -1, 0], reinforce_bs_2d[i, -1, 1], "gx", markersize=10
    )  # End point
